...
class TabButton: NSObject {
    
    // button数组
    var buttonArr:NSMutableArray = []
    
    //声明协议
    weak open var delegate: TabButtonDelagate?
    ...
    @objc func buttonClick(button:UIButton){
        for b in buttonArr{
            ...
            if (b as! UIButton) == button{
                ...
                if (self.delegate?.responds(to: #selector(TabButtonDelagate.clickChangePage(_:buttonIndex:))))!{
...
//协议的实现
@objc protocol  TabButtonDelagate : NSObjectProtocol{
    @objc func clickChangePage(_ tabButton:TabButton, buttonIndex:NSInteger)
}
